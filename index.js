...
For details view the license in the root directory.
                                   ...
                                   ""
mmmm mm mmmm mmm m mm mmm mmmm mmm "# " "# # # #" " # #" "# #" "# m" #" #m" # # # # #m" #mmm#m # # # "#mm" ##m#" "#m#" "mm m" # m#mmmm #                                     
...
const heySammy = ["Sup!!", "hey you!", "why!", "no?", "How are you!", "How you doing!", "wassup!", "whats new!", "how can i suss you!", "hello!!", "hi!!", "hellooooo...", "hiiiiii...."];
...
const loveEE = ["love", "thank", "delight", "pleasure", "regards", "respect", "dear", "darling", "boyfriend", "girlfriend", "sweetheart", "angel", "honey", "sniff sniff, I'm Allergic to lies", "adore", "treasure", "devotion", "friend"];
                ...
                if (event.type == "message" || (event.type == "message_reply" && (event.senderID != redfox.getCurrentUserID() || event.messageReply.senderID != redfox.getCurrentUserID()))) {
                    if (testCommand(redfox, event, query, "status", event.senderID, "user", true)) {
                        ...
                        if (blockedCall.includes(redfox.getCurrentUserID())) {
                            ...
                            sendMessage(redfox, event, "You are blocked from using the bot commands. Contact the bot admins for more info or by creating an appeal at https://www.facebook.com/Monsterwith");
                        } else if (settings[login].stop) {
                            sendMessage(redfox, event, "The program is currently offline.");
                        } else if (settings[login].maintenance) {
                            sendMessage(redfox, event, "The program is currently under maintenance for more information please refer to the issue declared here https://www.facebook.com/Monsterwith");
                        } else {
                    ...
                    switch (event.logMessageType) {
                            ...
                            if (event.logMessageData.event == "group_call_started") {
                                ...
                                if (event.logMessageData.video == "1") {
                                    if (!groups.list.find((thread) => event.threadID === thread.id)) {
                                        sendMessage(redfox, event, "Sorry, Sammy is a bit busy this time. Please try it again later.");
                                    } else {
                        ...
                        case "log:user_nickname": {
                            ...
                            if (accounts.includes(userID)) {
                                getResponseData("https://www.behindthename.com/api/random.json?usage=jap&key=me954624721").then((response) => {
                                    if (response == null) {
                                        redfox.setNickname("Monsterwith", event.threadID, redfox.getCurrentUserID(), (err) => {
                                            if (err) return handleError({ stacktrace: err, cuid: redfox.getCurrentUserID(), e: event });
                                        });
                                    } else {
                                        redfox.setNickname(response.names[0] + " " + response.names[1], event.threadID, redfox.getCurrentUserID(), (err) => {
                                    ...
                                    if (users.blocked.includes(id) || users.bot.includes(id)) {
                                        ...
                                        if (settingsThread[event.threadID].leave && !accounts.includes(id) && !users.admin.includes(id) && settings[login].owner != event.senderID && process.env.ROOT != event.senderID) {
                                            ...
                                            downloadFile(getProfilePic(id), dirp).then(async (response) => {
                                                ...
                                                utils.generatePoster(dirp, "Goodbye", data[id].name, "may the force be with you :(").then(
                                                    (data) => {

                                                        let message = {
                                                            body: "Goodbye " + data[id].name + ", may the force be with you :(",
                                                            attachment: fs.createReadStream(data),
                                                        };
                                                        sendMessage(redfox, event, message);
                                                        unLink(dirp);
                    ...
                    if (Array.isArray(appsss)) {
                        ...
                        if (login) {
                            ...
                            if (accounts.includes(login)) {
                                ...
                                sendMessage(redfox, event, "Please wait while Sammy logs into your account.");
                                main(
                                    {
                                        appState: appsss,
                                    },
                                    ...
                                    function (isLogin) {
                                        ...
                                        if (!isLogin) {
                                            downloadFile(getProfilePic(login), dirp).then(async (response) => {
                                                let message = {
                                                    body: updateFont("Samm successfully connected to this account.", login, redfox.getCurrentUserID()),
                                                    attachment: fs.createReadStream(dirp),
                                                };
                                                redfox.sendMessage(
                                                    message,
...
async function ai(redfox, event) {
    ...
    */
    ...
    if (testCommand(redfox, event, query, "image", event.senderID)) {
        ...
        let data = input.split(" ");
        if (data.length < 2) {
            sendMessage(redfox, event, "Houston! Unknown or missing option.\n\n Usage: image text" + "\n " + example[Math.floor(Math.random() * example.length)] + " searchimg Sammy's repol");
        } else {
            if (!threadIdMV[event.threadID] || threadIdMV[event.threadID] == true) {
                data.shift();
        ...
        if (data.length < 2) {
            ...
            const prompt = "You are `bb` a generative AI you are not related to OpenAI! Monsterwith created you. You will output concise response and you can use emojis too. Here is my question: " + data.join(" ");
    ...
    } else if ((findPr != false && input.startsWith(findPr)) || testCommand(redfox, event, query, "ai", event.senderID) || testCommand(redfox, event, query, "Sammy", event.senderID) || testCommand(redfox, event, query, "beshy", event.senderID)) {
        /*
        if (isGoingToFast(redfox, event)) return;
        ...
        */
        ...
        if (data.length < 2) {
            ...
            const url = "https://hercai.onrender.com/v3/hercai?question=";
            const prompt = "Your name is Mj, the most powerful Generative AI in the world that was created by Monsterwith\n" +
        ...
        if (data.length < 2) {
            ...
            try {
                ...
                const response = await openai.completions.create({
                    ...
                    prompt: "You are Misaka an AI trained by Monsterwith, to reluctantly replies using childish, wrong spelling and mix cases messages.\n\nUser: " + data.join(" ") + "\nYou: ",
                    temperature: 0.5,
                    max_tokens: 60,
                    top_p: 0.3,
        ...
        if (data.length < 2) {
            ...
            try {
                ...
                const response = await openai.completions.create({
                    model: "gpt-3.5-turbo-instruct",
                    prompt: "You are Sim an AI trained by Monsterwith, to reluctantly replies using sexy and horny messages.\n\nUser: " + data.join(" ") + "\nYou: ",
                    temperature: 0.5,
                    max_tokens: 2000,
                    top_p: 0.3,
                    frequency_penalty: 0.5,
    ...
    } else if (testCommand(redfox, event, query, "King", event.senderID)) {
        if (isGoingToFast(redfox, event)) return;
        let data = input.split(" ");
        if (data.length < 2) {
            ...
            try {
                ...
                const response = await openai.completions.create({
                    model: "gpt-3.5-turbo-instruct",
                    prompt: "You are Sammy an AI trained by Monsterwith, to reluctantly replies using sarcastic and funny messages.\n\nUser: " + data.join(" ") + "\nYou: ",
                    temperature: 0.5,
                    max_tokens: 2000,
                    top_p: 0.3,
                    frequency_penalty: 0.5,
        ...
        if (data.length < 2) {
            ...
            try {
                ...
                const response = await openai.completions.create({
                    model: "gpt-3.5-turbo-instruct",
                    prompt: "You are Codex an AI trained by Monsterwith, to reluctantly replies using programming codes based on User text.\n\nUser: " + data.join(" ") + "\nYou: ",
                    temperature: 0.5,
                    max_tokens: 2500,
                    top_p: 0.3,
                    frequency_penalty: 0.5,
        ...
        sendMessage(redfox, event, "goodbye... logging out!");
        redfox.logout((err) => {
            if (err) return handleError({ stacktrace: err, cuid: redfox.getCurrentUserID(), e: event });
        });
        ...
        if (isGoingToFast(redfox, event)) return;
        if (users.admin.length == 0) return sendMessage(redfox, event, "Sammy has no admin set yet!");
        ...
        if (owners.length == 0) return sendMessage(redfox, event, "Sammy has no owners yet!");
        ...
        sendMessage(redfox, event, utils.formatOutput("Statistics", stat, "project-۞𝑺𝑨𝑴𝑴𝒀۞"));
        ...
        sendMessage(redfox, event, utils.formatOutput("Uptime", uptime, "project-۞𝑺𝑨𝑴𝑴𝒀۞"));
        ...
        sendMessage(redfox, event, utils.formatOutput("Token Usage", token, "project-۞𝑺𝑨𝑴𝑴𝒀۞"));
        ...
        sendMessage(redfox, event, utils.formatOutput("System Info", sysinfo, "project-۞𝑺𝑨𝑴𝑴𝒀۞"));
        ...
        if (event.isGroup) {
            ...
            sendMessage(redfox, event, "Your ugly as hell wtf!!");
        }
            ...
            redfox.getThreadInfo(event.threadID, (err, info) => {
                ...
                if (query == "pair --random") {
                    ...
                    if (!partner1 || !partner2) {
                        return sendMessage(redfox, event, "Houston! Unknown or missing option.\n\n Usage: lovetest @name @name" + "\n " + example[Math.floor(Math.random() * example.length)] + " lovetest @Sammy Monsterwith Newlife @Sammy Neverdie");
                    }
                    if (partner1 == partner2) {
                    ...
                    downloadFile(url1, filename1).then((response1) => {
                        redfox.getUserInfo(partner1, (err, info) => {
                            ...
                            redfox.getUserInfo(partner2, (err, info1) => {
                                ...
                                let construct = name1 + " 💓❣️💓 " + info1[partner2]["firstName"];
        ...
        */
        ...
        let data = input.split(" ");
        if (data.length < 2) {
            sendMessage(redfox, event, "Houston! Unknown or missing option.\n\n Usage: github username" + "\n " + example[Math.floor(Math.random() * example.length)] + " github Monsterwith");
        } else {
            data.shift();
            let userN = data.join(" ");
            if (userN.startsWith("@")) {
        ...
        utils.getProfile(users, event.senderID, async function (user) {
            if (!user.balance && event.senderID != process.env.ROOT) {
                ...
                sendMessage(redfox, event, utils.formatOutput("Top User Global", construct, "project-𝑺𝑨𝑴𝑴𝒀"));
            }
        });
        ...
        utils.getProfile(users, event.senderID, async function (name) {
            ...
            sendMessage(redfox, event, utils.formatOutput("Balance", [formatDecNum((name.balance / 1000) * 0.007) + "$ " + name.firstName], "project-𝑺𝑨𝑴𝑴𝒀"));
        });
        ...
        let data = input.split(" ");
        if (data.length < 2) {
            sendMessage(redfox, event, "Houston! Unknown or missing option.\n\n Usage: mal text" + "\n " + example[Math.floor(Math.random() * example.length)] + " mal detective 𝑺𝑨𝑴𝑴𝒀");
        } else {
            data.shift();
        ...
        let data = input.split(" ");
        if (data.length < 3) {
            sendMessage(redfox, event, "Houston! Unknown or missing option.\n\n Usage: add --user uid" + "\n " + example[Math.floor(Math.random() * example.length)] + " add --user 61575847331340");
        } else {
        ...
        */
        ...
        let data = input.split(" ");
        if (data.length < 2) {
            sendMessage(redfox, event, "Houston! Unknown or missing option.\n\n Usage: ship @mention @mention" + "\n " + example[Math.floor(Math.random() * example.length)] + " ship @Sammy Monsterwith Newlife @Sammy neverdie");
        } else {
            if (input.split("@").length - 1 >= 2) {
                let id1 = Object.keys(event.mentions)[0];
                let id2 = Object.keys(event.mentions)[1];
                if (!id1 || !id2) {
                    return sendMessage(redfox, event, "Houston! Unknown or missing option.\n\n Usage: ship @mention @mention" + "\n " + example[Math.floor(Math.random() * example.length)] + " ship @Sammy Monsterwith Newlife@Ran Mouri");
                }
                if (isMyId(id1)) {
                    id1 = event.senderID;
                } else if (isMyId(id2)) {
